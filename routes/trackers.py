...
# create trackerid/items post route to create new item
...
def create_item(id):
    ...
    item = Item(name=name, description=description, descriptionFormat="Wiki", storyPoints=storyPoints, tracker_id=tracker.id, status=[status], assignedTo=[user])
    db.session.add(item)
    db.session.commit()
